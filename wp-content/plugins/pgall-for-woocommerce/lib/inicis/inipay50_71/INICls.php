...
class INILog {
	...
	function INILog($request) {
		...
		$this->starttime = InicisGetMicroTime();
	...
	function CloseLog($msg) {
		...
		$laptime = InicisGetMicroTime() - $this->starttime;
...
class INIData {
	...
	function ParsePIEncrypted() {
		if (version_compare(PHP_VERSION, '7.2.0', '>=')) {
			...
			$this->m_PIPGPubSN = $output['CertVer'];
			$this->m_PG1 = $output['pg1'];
			$this->m_PG2 = $output['pg2'];
			$this->m_PG1IP = $output['pg1ip'];
			$this->m_PG2IP = $output['pg2ip'];
		} else {
			parse_str($this->m_REQUEST["encrypted"]);
